...
export default async function ({ call_id, ws, log, args }) {
  ...
  if (typeof lat !== 'number' || typeof lon !== 'number' || isNaN(lat) || isNaN(lon)) {
    ws.send(JSON.stringify({
      ...
      item: { type: 'function_call_output', call_id, output: JSON.stringify({ error: 'Missing or invalid lat/lon for get_24h_forecast' }) }
    }));
    return;
  }
  try {
    ...
    ws.send(JSON.stringify({
      type: 'conversation.item.create',
      item: { type: 'function_call_output', call_id, output: JSON.stringify({ error: 'Failed to fetch 24h forecast' }) }
    }));
  }
}
